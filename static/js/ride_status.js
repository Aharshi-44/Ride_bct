(function () {
  ...
  function highlightStages(status) {
    ...
    document.querySelectorAll('#stage-labels [data-stage]').forEach(function (el) {
      ...
      el.classList.toggle('text-violet-600', i <= idx && idx >= 0);
      el.classList.toggle('dark:text-violet-400', i <= idx && idx >= 0);
      el.classList.toggle('text-slate-500', i > idx || idx < 0);
    });
  }
  ...
  function tick() {
    fetch(url, { headers: { 'X-Requested-With': 'XMLHttpRequest' }, credentials: 'same-origin' })
      ...
      .then(function (data) {
        ...
        if (badge) badge.textContent = labels[st] || st;
